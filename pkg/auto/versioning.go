...
func (b *Bump) calculateNextVersion(ctx context.Context, cfg *config.Config, versionOverride string, newChart bool) error {
	logger.Log(ctx, slog.LevelInfo, "calculate next version")

	if newChart {
		if err := b.netNewVersion(cfg); err != nil {
			return err
		}
	} else {
		...
		if err := b.applyVersionRules(cfg, versionOverride); err != nil {
			return err
		}
...
func (b *Bump) netNewVersion(cfg *config.Config) error {
	b.versions = &versions{
		toRelease:           &version{},
	...
	b.versions.toReleaseRepoPrefix.txt = strconv.Itoa(cfg.GetCurrentMajorPrefix(b.target.branchLine)) + ".0.0"
...
func getLatestVersionFromIndex(versions []string) (string, string, bool, error) {
...
func getLatestVersionRecursively(versions []string) string {
	if len(versions) == 0 {
		return ""
	}
...
func getCurrentRCsFromIndex(versions []string, toReleasePrefix string) ([]rc, error) {
	if !strings.Contains(versions[0], toReleasePrefix) || toReleasePrefix == "" {
		return nil, nil
	}

	currentRCs := make([]rc, 0)
	for _, v := range versions {
		if !strings.Contains(v, toReleasePrefix) {
			break
		}
		...
		prefix, app, found := parseRepoPrefixVersionIfAny(v)
		if !found {
...
// returns:
//   - toReleaseRepoPrefix
func (b *Bump) applyVersionRules(cfg *config.Config, versionOverride string) error {

	// get the repository major prefix version rule (i.e., 105; 104; 103...)
	repoPrefixVersionRule := strconv.Itoa(cfg.GetCurrentMajorPrefix(b.target.branchLine))
	repoPrefixVersionRule = repoPrefixVersionRule + ".0.0"
	...
	* in each case, the repoPrefixVersion will be bumped to 105.0.0
	 */
	if b.versions.latestRepoPrefix.txt == "" ||
		(repoPrefixSemverRule.Major-b.versions.latestRepoPrefix.svr.Major == 1) {
	...
	if (repoPrefixSemverRule.Major-b.versions.latestRepoPrefix.svr.Major > 1) ||
		...
		// something is terribly wrong, should not jump branch lines
		return errors.New("difference between major versions is more than 1 or repoPrefix version is lower than latestRepoPrefix")
	}
