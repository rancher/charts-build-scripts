...
func ArchiveCharts(ctx context.Context, specificChart string) error {
	...
	zipChart := func(ctx context.Context, fs billy.Filesystem, helmChartPath string, isDir bool) error {
		...
		chartVersionPath, err := filesystem.MovePath(ctx, helmChartPath, config.PathChartsDir, "")
		if err != nil {
		...
		foundChart = true
		chartAssetsDirpath := filepath.Join(config.PathAssetsDir, filepath.Dir(chartVersionPath))
		tgzPath, err := GenerateArchive(ctx, cfg.RootFS, fs, helmChartPath, chartAssetsDirpath, nil)
		if err != nil {
		...
		currentAsset, err := filesystem.MovePath(ctx, tgzPath, config.PathAssetsDir, "")
		if err != nil {
			return err
		}
		if err := DumpAssets(ctx, currentAsset); err != nil {
			return fmt.Errorf("encountered error while trying to update chart %s based on %s: %s", chartVersionPath, tgzPath, err)
		}
		return nil
	}

	if err := filesystem.WalkDir(ctx, cfg.RootFS, config.PathChartsDir, config.IsSoftError(ctx), zipChart); err != nil {
		return fmt.Errorf("encountered error while trying to zip charts: %s", err)
	}
	if len(specificChart) > 0 && !foundChart {
		return fmt.Errorf("could not find chart at %s/%s in repository", config.PathChartsDir, specificChart)
	}
...
func DumpAssets(ctx context.Context, specificAsset string) error {
	cfg, err := config.FromContext(ctx)
	if err != nil {
		return err
	}
	...
	unzipAsset := func(ctx context.Context, fs billy.Filesystem, tgzPath string, isDir bool) error {
		...
		assetPath, err := filesystem.MovePath(ctx, tgzPath, config.PathAssetsDir, "")
		if err != nil {
		...
		// Unarchive tgz
		chartChartsDirpath := filepath.Join(config.PathChartsDir, chart.Metadata.Name, chart.Metadata.Version)
	...
	if err := filesystem.WalkDir(ctx, cfg.RootFS, config.PathAssetsDir, config.IsSoftError(ctx), unzipAsset); err != nil {
		return fmt.Errorf("encountered error while trying to zip charts: %s", err)
	}

	if len(specificAsset) > 0 && !foundAsset {
		return fmt.Errorf("could not find asset at %s/%s in repository", config.PathAssetsDir, specificAsset)
	}
	return nil
}
