...
func ValidateRepository(rootFs billy.Filesystem, compareGeneratedAssetsOptions options.CompareGeneratedAssetsOptions, currentChart string) error {
	...
	if removeHelmIndex {
		defer utils.RemoveAll(rootFs, path.RepositoryHelmIndexFile)
	}
	...
	if err := utils.CopyDir(rootFs, path.RepositoryAssetsDir, newAssets); err != nil {
		return fmt.Errorf("Failed to copy %s into %s: %s", path.RepositoryAssetsDir, newAssets, err)
	}
	if err := utils.CopyDir(rootFs, path.RepositoryChartsDir, newCharts); err != nil {
	...
	packages, err = charts.GetPackages(utils.GetAbsPath(rootFs, path.ChartsRepositoryCurrentBranchDir), "")
	if err != nil {
